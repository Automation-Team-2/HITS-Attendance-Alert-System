...
Automated attendance monitoring system for Hindustan Institute of Technology and Science.
Detects students below 75% attendance and sends automated WhatsApp/SMS warnings.
...
| Frontend | HTML/CSS/JS → React 19 + Vite 6 + TypeScript (in progress) |
...
3. **Risk Detection** — Students with < 75% attendance are flagged
...
| `GET /api/students/` | List students with attendance % |
| `GET /api/attendance/section/{id}` | Section-wise attendance + risk flags |
